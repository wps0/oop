...
class MajorityGateTest {
    ...
    void givenNInputsShouldMajorityCorrectly(boolean[] inputs) {
        ...
        long trueCount = Arrays.stream(nonPrimitiveInputs).filter(val -> val).count();
        boolean expected = trueCount*2 > inputs.length;

        Circuit c = new Circuit();
        Gate gate = new MajorityGate();
